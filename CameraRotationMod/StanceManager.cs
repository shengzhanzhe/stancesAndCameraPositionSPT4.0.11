...
namespace CameraRotationMod
{
    ...
    public static class StanceManager
    {
        ...
        private static ConfigEntry<KeyboardShortcut> _stanceToggleKeyConfig;
        ...
        public static void Initialize(ConfigEntry<KeyboardShortcut> stanceToggleKeyConfig)
        {
        ...
        {
            // Check if the main stance toggle key is pressed (ignore modifiers to allow single key press)
            // Create a temporary KeyboardShortcut with only the main key (no modifiers)
            // This allows stance cycling even when holding other keys like movement keys
            var mainKeyOnly = new KeyboardShortcut(_stanceToggleKeyConfig.Value.MainKey);
            bool isKeyPressed = mainKeyOnly.IsDown();
            
            // Cycle through stances on key press (not hold)
